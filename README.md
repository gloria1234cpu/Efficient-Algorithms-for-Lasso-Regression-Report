...
The Lasso path gives the reverse perspective. When $\lambda$ is large, regularization is strong and only a few coefficients remain active. As $\lambda$ decreases, regularization becomes weaker and more coefficients enter the model. This matches the expected behavior of Lasso regression.
...
| Method | Objective | Nonzero | Error |
|---|---:|---:|---:|
| Our Coordinate Descent | 1.2976 | 28 | 0.7485 |
| sklearn Lasso | 1.2976 | 28 | 0.7485 |

Our Coordinate Descent implementation matches `sklearn.linear_model.Lasso` almost exactly, with the same objective value, number of nonzero coefficients, and recovery error. This validates the correctness of the courselib-based implementation.
